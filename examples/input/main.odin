...
import "../../kinetica/extensions/input"
...
create_depth_image :: proc(extent: vk.Extent2D) {
	using application
...
application_create :: proc() {
	using application
	context.logger = core.init_logger("./log.txt", .All, .Input)
	core.window_create(800, 600, "Depth example")
	input.init(core.window_get_handle())
	input.bind_key(input.Key{.Escape, {}, .Press}, close_window)
	...
	vertex_input_state := core.vk_vertex_input_state_create(
		{binding_description},
	...
	descriptor_pool = core.vk_descriptor_pool_create(
		{.UNIFORM_BUFFER},
		{Frames_In_Flight},
		Frames_In_Flight,
	)
...
application_run :: proc() {
	...
	for !core.window_should_close() {
		...
		if input.is_key_down(input.Key{.D, {}, .Press}) {
			rotation += f32(dt) * (glsl.PI / 4)
			if rotation >= glsl.PI * 2 do rotation = 0
		} else if input.is_key_down(input.Key{.A, {}, .Press}) {
			rotation -= f32(dt) * (glsl.PI / 4)
			if rotation <= 0 do rotation = glsl.PI * 2
		}
		ubo.model = glsl.mat4Rotate({0, 0, 1}, rotation)
		ubo.view = glsl.mat4LookAt({2, 2, -2}, {0, 0, 0}, {0, 1, 0})
		ubo.proj = glsl.mat4Perspective(
			glsl.radians_f32(45),
		...
		core.vk_command_scissor_set(command_buffers[frame], {{offset = {0, 0}, extent = extent}})

		core.vk_command_graphics_pipeline_bind(command_buffers[frame], pipeline)
		core.vk_command_descriptor_set_bind(
			command_buffers[frame],
			pipeline_layout,
			.GRAPHICS,
